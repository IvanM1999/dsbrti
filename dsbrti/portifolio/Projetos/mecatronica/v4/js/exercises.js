...
function normalizeText(str) {
  return String(str).replace(/\s+/g, " ").trim(); // CORREÇÃO: garante string
}
    ...
    generate() {
      ...
      return createExercise({
        ...
        solve: () => {
          ...
          if (!result || typeof result !== "object" || result.error) { // CORREÇÃO
            return EngineMath.result(null, result && result.error ? result.error : "Erro"); // CORREÇÃO
          }
          
          return EngineMath.result(
            result.value,
            String(result.display) + " A", // CORREÇÃO: garante string
            result.steps
          );
        },
...
export function getAdaptiveExercise(Progress) {
  ...
  const p = (Progress && typeof Progress.load === "function") ? Progress.load() : { topics: {} }; // CORREÇÃO
  const accRaw = (Progress && typeof Progress.accuracy === "function") ? Progress.accuracy() : 0; // CORREÇÃO
  const acc = Number.isFinite(Number(accRaw)) ? Number(accRaw) : 0; // CORREÇÃO
  ...
  for (let t in (p.topics || {})) {
    ...
    if (!topic || typeof topic.total !== "number" || topic.total === 0) continue; // CORREÇÃO
    
    const topicAcc = (topic.correct / topic.total) * 100;
    
    if (Number.isFinite(topicAcc) && topicAcc < worstAcc) { // CORREÇÃO
      worstAcc = topicAcc;
      weakest = t;
    }
  }
