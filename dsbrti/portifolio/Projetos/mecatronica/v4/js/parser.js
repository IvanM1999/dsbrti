...
export function parseCircuit(text) {
  ...
  for (let rawLine of lines) {
    ...
    if (typeof props.value !== "number" || !Number.isFinite(props.value)) { // CORREÇÃO
...
function detectType(name) {
  if (typeof name !== "string") return null; // CORREÇÃO
...
function parseProps(tokens) {
  ...
  if (!Array.isArray(tokens)) return props; // CORREÇÃO
  
  tokens.forEach(token => {
    
    if (typeof token !== "string") return; // CORREÇÃO
    
    const [key, val] = token.split("=");
    
    if (!key || val === undefined) return; // CORREÇÃO
    
    if (key === "value") {
      const num = parseFloat(val);
      props.value = Number.isFinite(num) ? num : null; // CORREÇÃO
    }
